...
def wiggle_3d_trefoil(savedir=None, show=True):
    ...
    axstereo.fig.set_size_inches(3, 3)
    if savedir is not None:
        axstereo.wiggle(savedir / "trefoil_3d_wiggle.gif", interval=50, frames=8, dpi=100)
...
def animate_2d_trefoil(savedir=None, show=True):
    ...
    ani = animation.FuncAnimation(
        axstereo.fig, animate, frames=np.arange(N_STEPS), interval=20, repeat=False
    )
    if savedir is not None:
...
def animate_3d_trefoil(savedir=None, show=True):
    ...
    ani = animation.FuncAnimation(
        axstereo.fig, animate, frames=np.arange(n_frames), interval=20, repeat=False
    )
    if savedir is not None:
